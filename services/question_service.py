...
def create_short_answer_question(
    ...
    # Find highest existing short-answer question_no for this exam
    query = (
        db.collection("questions")
        .where("exam_id", "==", exam_id)
    ...
    question_doc_id = f"{exam_id}-SA{next_no}"
...
def get_short_answer_questions_by_exam(exam_id: str) -> List[Dict]:
    query = (
        db.collection("questions")
        .where("exam_id", "==", exam_id)
        .where("question_type", "==", "SA")
...
def delete_short_answer_question(question_id: str) -> None:
    if not question_id:
        return
    ...
    for q in questions:
        ...
        new_doc_id = f"{exam_id}-SA{new_index}"
...
def has_short_for_exam(exam_id: str) -> bool:
    if not exam_id:
        return False
