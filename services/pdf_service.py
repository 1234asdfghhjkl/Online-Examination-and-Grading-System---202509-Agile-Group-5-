...
def generate_result_pdf(result_data: Dict) -> bytes:
    ...
    summary_data = [
        ...
        ["", ""],
        ["Total Score:", f"{result_data['overall_obtained']}/{result_data['overall_total']} ({result_data['overall_percentage']}%)"],
        ["MCQ Score:", f"{result_data['mcq_obtained']}/{result_data['mcq_total']}"],
        ["Short Answer Score:", f"{result_data['sa_obtained']}/{result_data['sa_total']}"],
    ]
    ...
    if result_data["mcq_results"]:
        ...
        for q in result_data["mcq_results"]:
            ...
            status_color = colors.HexColor('#22c55e') if q["is_correct"] else colors.HexColor('#dc3545')
            
            q_data = [
                [f"Question {q['question_no']}", f"{status} ({q['marks_obtained']}/{q['marks']} marks)"],
                [Paragraph(q['question_text'], styles['Normal']), ""],
                ["A. " + q['option_a'], ""],
                ["B. " + q['option_b'], ""],
                ["C. " + q['option_c'], ""],
                ["D. " + q['option_d'], ""],
                [f"Your Answer: {q['student_answer']}", f"Correct Answer: {q['correct_answer']}"],
            ]
    ...
    if result_data["sa_results"]:
        ...
        for q in result_data["sa_results"]:
            sa_data = [
                [f"Question {q['question_no']}", f"({q['awarded_marks']}/{q['max_marks']} marks)"],
                [Paragraph(q['question_text'], styles['Normal']), ""],
                ["Your Answer:", ""],
                [Paragraph(q['student_answer'] if q['student_answer'] != 'Not answered' else "No answer provided", 
                          styles['Normal']), ""],
            ]
