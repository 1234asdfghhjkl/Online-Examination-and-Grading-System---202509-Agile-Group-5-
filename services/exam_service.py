...
def check_grading_locked(exam_id: str) -> tuple[bool, str, dict]:
    ...
    if not deadline_date:
        return False, "No deadline set - grading always open", {
            "has_deadline": False,
            "legacy_mode": True
        }
    ...
    try:
        ...
        if is_locked:
            ...
            return True, f"Locked {days_passed}d {hours_passed}h ago", {
            ...
            return False, f"{days_remaining}d {hours_remaining}h remaining", {
                "deadline": deadline_dt.strftime("%Y-%m-%d %H:%M"),
        ...
        return True, f"Invalid deadline format: {e}", {
