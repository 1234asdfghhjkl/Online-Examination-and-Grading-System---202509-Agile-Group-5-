...
def save_exam_filters(exam_id: str, filters: Dict[str, List[str]]) -> None:
    ...
    print(f"✅ DEBUG - Verified saved in Firestore: {saved_data.get('student_filters')}")
...
def get_available_filters() -> Dict[str, List[str]]:
    ...
    try:
        ...
        result = {
            ...
            "semesters": sorted(list(semesters), key=lambda x: int(x) if x.isdigit() else 0)
        }
        ...
        return {
            "years": ["1", "2", "3", "4"],
            "majors": ["Computer Science", "Mechanical Engineering", "Business Administration"],
            "semesters": ["1", "2"]
        }
...
def is_student_eligible(student_id: str, exam_id: str) -> bool:
    ...
    if (not filters.get("years") and 
        not filters.get("majors") and 
        not filters.get("semesters")):
        return True
    
    student_ref = db.collection("users").where("student_id", "==", student_id).limit(1).stream()
    ...
    if filters.get("years"):
        if student_year not in filters["years"]:
            print(f"❌ Student {student_id} year {student_year} not in {filters['years']}")
            return False
    
    if filters.get("majors"):
        if student_major not in filters["majors"]:
            print(f"❌ Student {student_id} major {student_major} not in {filters['majors']}")
            return False
    
    if filters.get("semesters"):
        if student_semester not in filters["semesters"]:
            print(f"❌ Student {student_id} semester {student_semester} not in {filters['semesters']}")
            return False
