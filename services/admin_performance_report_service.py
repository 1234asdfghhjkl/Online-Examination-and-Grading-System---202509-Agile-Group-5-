...
def get_exam_performance_report(exam_id: str) -> Optional[Dict]:
    ...
    # Get all submissions
    submissions_query = db.collection("submissions").where("exam_id", "==", exam_id).stream()
...
def calculate_short_answer_statistics(submissions: List[Dict]) -> Dict:
    ...
    valid_sa = [(s, t, p) for s, t, p in zip(sa_scores, sa_totals, sa_percentages) if t > 0]
...
def calculate_question_performance(exam_id: str, submissions: List[Dict]) -> Dict:
    ...
    for q in mcq_questions:
        ...
        for sub in submissions:
            ...
            q_result = next((r for r in question_results if r.get("question_no") == q_no), None)
        ...
        question_stats[f"Q{q_no}"] = {
            ...
            "success_rate": round((correct_count / total_responses * 100) if total_responses > 0 else 0, 2),
            "max_marks": max_marks,
        }
...
def get_top_performers(submissions: List[Dict], limit: int = 10) -> List[Dict]:
    ...
    sorted_submissions = sorted(
        submissions,
        key=lambda x: x.get("overall_percentage", 0),
        reverse=True
    )
...
def identify_weak_areas(submission: Dict) -> List[str]:
    ...
    mcq_percentage = (
        ...
        if submission.get("mcq_total", 0) > 0 else 0
    )
    
    sa_percentage = (
        (submission.get("sa_obtained_marks", 0) / submission.get("sa_total_marks", 1)) * 100
        if submission.get("sa_total_marks", 0) > 0 else 0
    )
