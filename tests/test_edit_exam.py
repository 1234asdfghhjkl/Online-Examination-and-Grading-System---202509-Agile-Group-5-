...
class ExamEditTest(unittest.TestCase):
    ...
    def test_get_exam_edit_success(self, mock_get_exam_by_id, mock_render):
        ...
        self.assertEqual(
            rendered_template,
            "exam_edit.html",
            "Should render the exam edit form ('exam_edit.html')",
        )
        self.assertEqual(context["title"], MOCK_EXAM_DATA["title"])
        self.assertEqual(
            context["start_time"],
            MOCK_EXAM_DATA["start_time"],
            "Context should include start_time",
        )
        self.assertEqual(
            context["end_time"],
            MOCK_EXAM_DATA["end_time"],
            "Context should include end_time",
        )
    ...
    def test_get_exam_edit_missing_id(self, mock_get_exam_by_id, mock_render):
        ...
        self.assertEqual(
            status_code, 400, "Should return 400 Bad Request for missing ID"
        )
        mock_get_exam_by_id.assert_not_called()

        rendered_template, context = mock_render.call_args[0]
        self.assertEqual(
            rendered_template, "exam_edit.html", "Should render 'exam_edit.html'"
        )

        self.assertIn(
            "Error: Exam ID is missing",
            context["errors_html"],
            "The errors_html context should contain the error message",
        )
    ...
    def test_get_exam_edit_not_found(self, mock_get_exam_by_id, mock_render):
        ...
        self.assertIn(
            "alert alert-danger",
            context["errors_html"],
            "Should include an error message in errors_html context",
        )
    ...
    @patch("web.exams.render")
    def test_post_exam_edit_success(
        self,
        mock_render,
        mock_validate_times,
        mock_validate_date,
        ...
        self.assertEqual(
            rendered_template,
            "exam_edit.html",
            "Should render the exam edit page ('exam_edit.html')",
        )

        self.assertIn(
            "alert alert-success",
            context["success_html"],
            "Should include the success message HTML",
        )
        self.assertEqual(
            context["errors_html"], "", "Should not include errors_html on success"
        )
        ...
        self.assertEqual(
            context["duration"],
            "100",
            "Duration should be echoed back as a string from the form",
        )
        self.assertEqual(
            context["start_time"],
            "09:00",
            "Start time should be echoed back from the form",
        )
        self.assertEqual(
            context["end_time"], "10:40", "End time should be echoed back from the form"
        )
    ...
    @patch("web.exams.validate_exam_times", return_value=[])
    @patch("web.exams.render")
    def test_post_exam_edit_validation_failure(
        self,
        mock_render,
        mock_validate_times,
        mock_validate_date,
        ...
        # 2. Assert HTTP Status
        self.assertEqual(
            status_code, 400, "Should return 400 Bad Request on validation failure"
        )
        ...
        self.assertEqual(
            rendered_template, "exam_edit.html", "Should re-render the exam edit page"
        )

        self.assertIn(
            "alert alert-danger",
            context["errors_html"],
            "Should include the error message HTML",
        )
        self.assertIn(
            "Title is required.",
            context["errors_html"],
            "Should show the specific validation error",
        )
        self.assertEqual(
            context["success_html"], "", "Should not include success_html on failure"
        )

        # Check that context holds the invalid (echoed) data, including time
        self.assertEqual(
            context["exam_id"], VALID_EDIT_EXAM_ID
        )  # Extracted from the form body
        self.assertEqual(
            context["title"], "", "Invalid/missing title should be echoed back"
        )
        self.assertEqual(
            context["start_time"],
            "10:00",
