...
class StudentResultViewTest(unittest.TestCase):
    ...
    @patch(
        "web.student_result_routes.check_results_released",
        return_value=(False, "2025-12-15", "10:00"),
    )
    @patch(
        "web.student_result_routes.get_exam_by_id",
        return_value={"title": "Midterm Exam"},
    )
    def test_view_results_pending_with_date(
        self, mock_get_exam, mock_check_released, mock_render_fn
    ):
        """Test case where results are not yet released but a date is set (Expects 200)."""
        response, status = get_student_result_view(
            self.MOCK_EXAM_ID, self.MOCK_STUDENT_ID
        )
    ...
    def test_view_results_pending_no_date(
        self, mock_get_exam, mock_check_released, mock_render_fn
    ):
        ...
        self.assertIn("<h2>⏰ Results Not Yet Released</h2>", normalized_response)
        self.assertIn(
            "The instructor has not set a release date yet", normalized_response
        )

    @patch(
        "web.student_result_routes.check_results_released",
        return_value=(True, "2025-12-01", "00:00"),
    )
    @patch("web.student_result_routes.get_student_result", return_value=None)
    def test_view_no_submission_found(
        self, mock_get_result, mock_check_released, mock_render_fn
    ):
        ...
        self.assertIn(
            "<p>You have not submitted this exam yet.</p>", normalized_response
        )
    ...
    @patch(
        "web.student_result_routes.get_student_result",
        return_value=MOCK_SUCCESS_RESULT_DATA,
    )
    def test_view_success_content_check(
        self, mock_get_result, mock_check_released, mock_render_fn
    ):
    ...
    def test_pdf_results_not_released(self, mock_check_released, mock_render_fn):
        ...
        response, status, headers = get_student_result_pdf(
            self.MOCK_EXAM_ID, self.MOCK_STUDENT_ID
        )
    ...
    @patch("web.student_result_routes.get_student_result", return_value=None)
    def test_pdf_no_submission_found(
        self, mock_get_result, mock_check_released, mock_render_fn
    ):
    ...
    def test_pdf_success(
        self, mock_get_result, mock_check_released, mock_generate_pdf, mock_render_fn
    ):
        ...
        self.assertIn(
            'attachment; filename="result_Final_Python_Exam_S007.pdf"',
