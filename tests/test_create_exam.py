...
class ExamHandlerTest(unittest.TestCase):
    ...
    @patch("web.exams.render")
    def test_exam_submit_validation_failure(
        self, render, mock_validate_exam, mock_validate_date, mock_save_draft
    ):
    ...
    @patch(
        "web.exams.publish_exam"
    )  # Mock the function that changes the status to published
    @patch("web.exams.save_exam_draft", return_value="test_exam_103")
    @patch("web.exams.validate_exam_date", return_value=[])
    @patch("web.exams.validate_exam", return_value=[])
    @patch("web.exams.render")
    def test_exam_publish_success(
        self,
        render,
        mock_validate_exam,
        mock_validate_date,
